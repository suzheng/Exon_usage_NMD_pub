...
rule bam2fastq:
    ...
    shell:
        "samtools sort -n {input.bam} -o {output.nsorted_bam}; samtools fastq {output.nsorted_bam}|gzip -c >{output.reads1}"
...
rule align:
    ...
    output:
        ...
        Sorted_bam="STAR_Output/{sample}.Aligned.sortedByCoord.out.bam" #removed temp() for ggsashimi plot
    log:
    ...
    threads:  # Samtools takes additional threads through its option -@
        10     # This value - 1 will be sent to -@
    shell:
        "rm -rf $TMPDIR/star;STAR --runThreadN 10 --outFilterMismatchNmax 2 --outFilterMultimapNmax 1 --outSAMtype BAM Unsorted SortedByCoordinate --genomeDir " + ref_genome + " --outTmpDir $TMPDIR/star --readFilesIn {input.reads1}  --outFileNamePrefix STAR_Output/{wildcards.sample}. --readFilesCommand zcat" 
